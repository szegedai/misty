...
class SpeechToTextAPI():
    ...
    async def ws_wav_upload(self, websocket, wav_filename, wav_length, wav_nframes, bitrate):
        with open(wav_filename, "rb") as wav_file_bin:
            ...
            while nframe < length:
                frame = wav_file_bin.read(bitrate) # 16 bit -> 1 frame = 2 Bytes
                if nframe == length-1:
                    frame = wav_file_bin.read(residual)
    ...
    async def ws_wav_recognition(self, wav_filename, bitrate):
        async with websockets.connect(self.uri) as ws:
            try:
                ...
                await self.ws_wav_upload(ws, wav_filename, wav_length, wav_nframes, bitrate)
    ...
    if(len(sys.argv) > 2):
        ...
        try:
            if asyncio.run(stt_api.ws_check_connection()):
                res = asyncio.run(stt_api.ws_wav_recognition(wav_filename, 1024))
